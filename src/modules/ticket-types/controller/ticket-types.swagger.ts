...
export const SwaggerTicketTypeController = {
  Main: () => applyDecorators(ApiTags('Ticket Types'), ApiBearerAuth()),
      ...
      ApiOkResponse({
        ...
        schema: {
          properties: {
            data: {
              type: 'array',
              items: { $ref: getSchemaPath(TicketTypeResponseDto) },
            },
            meta: {
              type: 'object',
              properties: {
  ...
  Update: () =>
    applyDecorators(
      ApiOperation({
        ...
        description:
          'Permite editar preço e quantidade. Não permite reduzir a quantidade total para menos do que já foi vendido.',
      }),
      ApiOkResponse({ type: TicketTypeResponseDto }),
      ApiBadRequestResponse({
        description:
          'A nova quantidade é menor do que os ingressos já vendidos.',
      }),
  ...
  Delete: () =>
    applyDecorators(
      ...
      ApiBadRequestResponse({
        description: 'Não é possível excluir um lote que já possui vendas.',
      }),
      ApiNotFoundResponse({ description: 'Lote não encontrado.' }),
      ApiUnauthorizedResponse({
        description: 'Acesso negado. Autenticação necessária.',
