...
describe('UpdateUserUseCase', () => {
 ...
 describe('execute', () => {
  ...
  it('deve lançar ConflictException se o novo e-mail já estiver em uso', async () => {

   const updateDto = { email: 'outro@email.com' };
   mockUsersRepository.findById.mockResolvedValueOnce({ ...mockUser });

   const dbError = { code: '23505' };
   mockUsersRepository.save.mockRejectedValueOnce(dbError);


   await expect(useCase.execute(userId, updateDto)).rejects.toThrow(
    new ConflictException('Este e-mail já está em uso por outro usuário.')
   );
  });
  ...
  it('deve lançar InternalServerErrorException em erros genéricos', async () => {
   ...
   mockUsersRepository.findById.mockResolvedValueOnce({ ...mockUser });
   mockUsersRepository.save.mockRejectedValueOnce(new Error('Fatal fail'));


   await expect(useCase.execute(userId, { username: 'erro' })).rejects.toThrow(
    new InternalServerErrorException('Erro interno ao atualizar o usuário.')
   );
  });
 });
});